...
@migrate_group.command("youtube-transcripts")
@click.option("--cleanup", is_flag=True, default=False, help="Delete old raw/transcripts/*.md after successful migration.")
@click.option("--limit", type=int, default=None, help="Maximum number of videos to migrate (useful for testing).")
@click.pass_context
def migrate_youtube_transcripts_cmd(ctx: click.Context, cleanup: bool, limit: int | None) -> None:
    """Migrate raw/transcripts/<id>.md files to inbox/youtube/ new format.

    For each old transcript:
    - Fetches publish date, duration, views, and thumbnail via yt-dlp (falls back to
      empty values if yt-dlp fails for a given video).
    - Writes inbox/youtube/<date>-<slug>--<id>/index.md and original.json.
    - Writes the corresponding state file in .llmkb/state/sources/.
    - With --cleanup, removes the old raw/transcripts/<id>.md on success.

    Use --dry-run to preview what would be migrated without writing anything.
    Use --limit N to process only the first N candidates (good for spot-checking).
    """
    from cogforge.migrate_youtube import migrate_youtube_transcripts
    ...
    result = migrate_youtube_transcripts(
        paths,
        dry_run=dry_run,
        cleanup=cleanup,
        verbose=verbose,
    ...
    if cleanup:
        data["cleaned_up_ids"] = result.cleaned_up
    ...
    if output_format == "markdown":
        ...
        if cleanup:
            lines.append(f"**Cleaned up:** {result.summary['cleaned_up']}")
        if result.failed:
            lines.append("\n## Failed")
            for vid in result.failed:
                lines.append(f"- `{vid}`")
        click.echo("\n".join(lines))
    else:
        _echo_json(data)
